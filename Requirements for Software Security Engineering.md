...
![Diagram](https://github.com/UNO-CYBR-8420-Team1/CYBR8420-Suricata/blob/main/Use%20Cases/UserAuthenticationAccessControlUseMisuseCase.drawio.png)
...
From misuse case #2 we want to ensure that only authorized personnel can access Suricata. This does not come natively with Suricata, but it can be achieved by properly securing the host system that runs Suricata. Suricata also allows for access control of filesystems, which should be implemented as a best practice when configuring Suricata for deployment.

From misuse case #3 we should to ensure that the IPS rules contained within Suricata remain secure with an added logging mechanism being in place for this purpose. Additionally, further requirements, aside from authentication and user roles, would be further securing the logging and alerting mechanisms utilized by the IPS whether using techniques such as encryption or ensuring log immutability.
...
- Currently Suricata does contain documentation pertaining to [logging](https://docs.suricata.io/en/latest/devguide/extending/output/index.html) and [alerts](https://docs.suricata.io/en/latest/make-sense-alerts.html#) respectively. Logging is conducted in a number of different ways, some of which include logging for files, streams, and transactions. It is worth noting that transactional logging is experiencing issue as of the time of writing with custom logging scenarios as they outline. It is not clear as of this time if there are encryption or immutability approaches being taken for logging. In terms of alerting mechanisms, said alerts are provided by Suricata pertaining to specific rules present with instructions for ways to properly process the meaning behind alerts, potentially with an included vulnerability listing if applicable.
- Suricata does not come with user authentication. It is a CLI-based system, so there are no graphical interfaces for user login prompts. One way to strengthen Suricata’s security posture is by securing access of Suricata’s host system; per its documentation, this is typically a Linux instance. Locking down [file system permissions](https://docs.suricata.io/en/latest/security.html#running-as-a-user-other-than-root) would help decrease the risk of a random user or attacker gaining access to Suricata. Suricata’s documentation does warn that even with locking down different file systems, Suricata must be started with root privileges to obtain rules and capabilities for its upcoming runtime on a production network.
...
Grace - My individual contribution include brainstorming ideas for use/misuse cases with other teammates. I focused on reading suricata documentation and trying to understand basic functionality and researching the documentation for potential improvements. Then I focused on writing the review. I also focused on developing Use/Misuse Case #5, which included creating the associated diagram and description. Additionally, I assisted with filling out any required sections of the project to ensure completeness.

Nathan - I generated a bunch of initial use cases and misuse cases as a starting point while we were brainstorming ideas ~10. I reported issues clearly and with GITHUB issue feature. I generated an initial use case diagram to use with discussion of the professor to get feedback. I attended all the meetings. I stayed engaged in updates as promptly as I could. I made sure we left meetings with expectations of deadlines together. I helped put together the contributions initially and reflection section as a starting point, and some “refined security features” from our misuse scenario. 

Shane - For my individual contributions for this assignment I have worked on Use/Misuse case #3 as well as the written portion of the section covering AI prompting following discussing its use and implementation with the rest of the team. I have participated in helping come up with our final five use cases while together as a group. I have attended all group meetings and have worked to provide a list of To-Dos for us to keep track of for at a glance planning in Discord as well as handling the GitHub project board to reflect team planning and individually assigned tasks. Other minor tasks I have conducted include setting up the skeleton for the requirements for SSE document for later completion and assisting with the organizational structure and appearance of the document.

Matt - For this portion of the group project, I participated in use/misuse case brainstorming activities and provided two use/misuse cases – one of which I used for my contribution to this assignment’s deliverables. For the use/misuse case, I provided a use/misuse case diagram and descriptions of the use case, misuse case, and ways to mitigate the misuse case. As I began to dive deeper into my use/misuse case, I downloaded Suricata to my PC and read through Suricata’s documentation on how the software functions. This helped me provide a robust use/misuse case and improved my mitigation strategies for the misuse case. I participated in the scheduled team meetings and check-in with Dr. Gandhi and actively participated in the group’s Discord chat to ask questions, provide my thoughts to others, and inform others of my progress on my deliverables. I reviewed others’ submissions for basic errors and quality checks.  
...
We were able to brainstorm together numerous use / misuse cases but what we needed to do as stressed by professor was to focus on external actors vs internal because there isn't much you can do about a disgruntled worker. We worked together to narrow down our five use / miususe cases that best supported this milestone in our project so we had the big hitters vs more minor ones we had chosen. As always the continued collaboration on discord (text and voice) paired with the project board helped us better tackle individual and group contributions.
